...
import com.dhc.plugin.psi.psiUtil.getChildrenOfType
import com.dhc.plugin.ui.ConfigureDialogWithModulesAndVersion.COMPVERSION
...
class GroovyBuildScriptManipulator(private val groovyScript: GroovyFile) : GradleBuildScriptManipulator {
    ...
    override fun configureModuleBuildScript(
        ...
        if (!containsDirective(groovyScript.text, applyPluginDirective)) {//判断是都包含apply
            ...
            if (!applyStatement.isEmpty()) {
                replaceApply(applyStatement, applyPluginDirective)
...
//        }
    ...
    private fun replaceApply(applyStatement: List<GrApplicationStatement>, applyPluginDirective: String) {
        for (stmt in applyStatement) {
            val isCanReplace = stmt.getNode().text.contains("com.android.application")
                    || stmt.getNode().text.contains("com.android.library")
            if (isCanReplace) {
                stmt.replaceWithStatementFromText(applyPluginDirective)
            }
        }
    }
    ...
    override fun configureProjectBuildScript(version: String): Boolean {
        ...
        groovyScript.apply {
            ...
            getBuildScriptDependenciesBlock().apply {
                addLastExpressionInBlockIfNeeded(CLASSPATH.replace(VERSION_TEMPLATE,COMPVERSION))
            }
        }
    ...
    override fun getKotlinStdlibVersion(): String? {
        val versionProperty = "\$component_version"
        groovyScript.getBlockByName("buildScript")?.let {
            if (it.text.contains("ext.component_version = ")) {
...
//        }
    ...
    private fun getARouterGroovyDependencySnippet(artifactName: String) ="annotationProcessor \"com.alibaba:$artifactName:1.1.2.1\""//arouter-compiler
    private fun getDagger2GroovyDependencySnippet(artifactName: String) ="annotationProcessor \"com.google.dagger:$artifactName:2.13\""//dagger-compiler
    ...
    private fun getApplyStatement(file: GroovyFile): List<GrApplicationStatement> =
        file.getChildrenOfType<GrApplicationStatement>().filter {
            it.invokedExpression.text == "apply"
        }
    ...
    private fun GrClosableBlock.addRepository(): Boolean {
        val repository = getRepositoryForVersion()
        val snippet = when {
    ...
    companion object {
        ...
        private val VERSION = String.format("ext.component_version = '%s'", VERSION_TEMPLATE)
        private val GRADLE_PLUGIN_ID = "component-plugin"
        private val CLASSPATH = String.format( "classpath \"$KOTLIN_GROUP_ID:${GRADLE_PLUGIN_ID}:%s\"", VERSION_TEMPLATE)
    }
}